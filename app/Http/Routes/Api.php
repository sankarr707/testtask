...
$router->group(['prefix' => 'mailchimp', 'namespace' => 'MailChimp'], function () use ($router) {
    ...
    $router->group(['prefix' => 'lists'], function () use ($router) {
        ...
        $router->post('/{listId}/members', 'ListsMembersController@add');
        $router->put('/{listId}/members/{memberId}', 'ListsMembersController@update');
        $router->delete('/{listId}/members/{memberId}', 'ListsMembersController@remove');
    });
