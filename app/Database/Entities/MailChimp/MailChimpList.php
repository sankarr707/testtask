...
class MailChimpList extends MailChimpEntity
{
    ...
    /**
     * @ORM\OneToMany(targetEntity="App\Database\Entities\MailChimp\MailChimpListMembers", mappedBy="memberList")
     */
    private $mailChimpListMembers;
    ...
    {
        return $this->mailChimpListMembers;
    }

    public function addMembers(MailChimpListMembers $members): self
    {
        if (!$this->mailChimpListMembers->contains($members)) {
            $this->mailChimpListMembers[] = $members;
            $members->setMemberList($this);
        }
        return $this;
    }

    public function removeMembers(MailChimpListMembers $members): self
    {
        if ($this->mailChimpListMembers->contains($members)) {
            $this->mailChimpListMembers->removeElement($members);
            // set the owning side to null (unless already changed)
            if ($members->getMemberList() === $this) {
                $members->setMemberList(null);
            }
        }
        return $this;
    }
